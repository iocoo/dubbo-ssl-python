...
from dubbo_ssl.codec.decoder import parse_response_head, Response
from dubbo_ssl.codec.encoder import Request
from dubbo_ssl.common.constants import CLI_HEARTBEAT_RES_HEAD, CLI_HEARTBEAT_TAIL, TIMEOUT_IDLE, TIMEOUT_MAX_TIMES, \
    CLI_HEARTBEAT_REQ_HEAD, TIMEOUT_CHECK_INTERVAL
from dubbo_ssl.common.exceptions import DubboRequestTimeoutException, DubboResponseException
from dubbo_ssl.common.util import DEFAULT_READ_PARAMS, get_invoke_id
...
class BaseConnectionPool(object):
    ...
    def get(self, host, request_param, timeout=None, verify=None):
        """
        执行远程调用获取数据
        """
        conn = self._get_connection(host,verify)
    ...
    def _new_connection(self, host, verify):
        raise NotImplementedError()

    def _delete_connection(self, conn):
        raise NotImplementedError()

    def _get_connection(self, host, verify):
        if not host and ':' not in host:
            raise ValueError("invalid host {}".format(host))
        if host not in self._ssl_config:
            self._ssl_config[host] = {}
            self._ssl_config[host]["verify"] = verify
    ...
    def _parse_head(self, data, conn):
        ...
        if heartbeat == 2:
            ...
            host = conn.remote_host()
            if host in self.client_heartbeats and self.client_heartbeats[host] >0:
    ...
    def _send_heartbeat(self):
        ...
        while 1:
            ...
            for host in self._connection_pool.keys():
                try:
                    self._check_conn(host,self._ssl_config[host]["verify"] if host in self._ssl_config else None)
                except Exception as e:
                    logger.error(e)
            ending = time.time()
    ...
    def _check_conn(self, host, verify):
        """
...
class SelectConnectionPool(BaseConnectionPool):
    ...
    def _new_connection(self, host, verify):
        ip, port = host.split(':')
        self._connection_pool[host] = Connection(ip, int(port), verify)
        # 保证select模型已经开始监听最新加入的这个fd的读事件，否则可能会导致此fd读事件丢失
        time.sleep(self.select_timeout)

    def _delete_connection(self, conn):
        host = conn.remote_host()
        if host in self._connection_pool:
...
class Connection(object):
    ...
    def __init__(self, server_ip, port, verify) -> None:
        ...
        if verify is None:
            ...
            self.__sock = context.wrap_socket(sock,server_hostname=server_ip)
        ...
        with self._write_lock:
            while data:
                try:
                    ...
                    if isinstance(e,BlockingIOError) or e.errno in (35,11):
                        time.sleep(.01)
                    else:
                        raise
    ...
    def read(self, callback):
        """
        读取远程主机的数据,支持SSL，解决缓存和粘包半包问题
        """
        self.last_active = time.time()

        while True:
            # 1. 如果有数据满足协议所需长度，截取并触发回调
            if len(self.read_buffer) >= self.read_length:
                consumed_length = self.read_length
                self.read_length, self.read_type, self.invoke_id = callback(bytearray(self.read_buffer[:consumed_length]), 
                                                                            self, self.read_type, self.invoke_id)
                
                self.read_buffer = self.read_buffer[consumed_length:]
                continue

            # 2. 计算还需要多少字节
            need_length = self.read_length - len(self.read_buffer)

            # 3. 尝试从 SSL 套接字中读取数据
            try:
                recv_data = self.__sock.recv(need_length)
            ...
            # 5. Append to the buffer,解决半包
            self.read_buffer.extend(recv_data)
